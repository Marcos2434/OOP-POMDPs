...
def create_line_pre(budget, target, size, threshold, det, pre):
	...
	for sub in states_obs:
		...
		for i in observales:
			if i in total:
				...
				total[i] = ''
	if det == 0:
		file = open('OOP/generated_models/fixed_line_' + str(size)  +'_ran_z3.py', 'w')
	else:
		file = open('OOP/generated_models/fixed_line_' + str(size) +'_det_z3.py', 'w')
	...
	file.write('# Expected cost/reard equations\n')
